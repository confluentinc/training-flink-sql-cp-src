...
export SCHEMA_REGISTRY_URL="${SCHEMA_REGISTRY_URL:-https://localhost:8081}"
export DELAY_MS="${DELAY_MS:-1000}"
export COUNT="${COUNT:-0}"

# TLS — lab CA bundle generated by setup-all.sh
export LAB_TRUSTSTORE="${LAB_TRUSTSTORE:-$HOME/.lab/truststore.jks}"
export LAB_TRUSTSTORE_PASSWORD="${LAB_TRUSTSTORE_PASSWORD:-changeit}"
LAB_CA_CRT="${LAB_CA_CRT:-$HOME/.lab/ca.crt}"

if [[ ! -f "$LAB_TRUSTSTORE" || ! -f "$LAB_CA_CRT" ]]; then
  echo "ERROR: lab TLS material not found ($LAB_TRUSTSTORE / $LAB_CA_CRT)" >&2
  echo "Run labs-src/scripts/setup-all.sh to provision it." >&2
  exit 1
fi
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo "  SCHEMA_REGISTRY_URL  (default: https://localhost:8081)"
      echo "  LAB_TRUSTSTORE       Path to lab JKS truststore (default: \$HOME/.lab/truststore.jks)"
...
flink_wait_statement_completed() {
  ...
  while [ $elapsed -lt $max_wait ]; do
    local response
    response=$(curl -s "${CURL_TLS[@]}" "$CMF_URL/cmf/api/v1/environments/$FLINK_ENV/statements/$stmt_name")
    local phase
...
setup_flink_tables() {
  ...
  if ! curl -s "${CURL_TLS[@]}" -f "$CMF_URL/cmf/api/v1/environments/$FLINK_ENV" > /dev/null 2>&1; then
