...
SCHEMA_REGISTRY_URL="${SCHEMA_REGISTRY_URL:-https://localhost:8081}"
AVRO_PRODUCER_COMMAND="kafka-avro-console-producer"

# TLS (lab CA truststore — generated by setup-all.sh)
LAB_TRUSTSTORE="${LAB_TRUSTSTORE:-$HOME/.lab/truststore.jks}"
...
if [[ ! -f "$LAB_TRUSTSTORE" ]]; then
  ...
  echo "Run labs-src/scripts/setup-all.sh first to provision it." >&2
  exit 1
fi
...
EOF
  ...
  if printf '%s|%s\n' "$KEY" "$VALUE" | \
     ...
     "$AVRO_PRODUCER_COMMAND" \
       ...
       --property schema.registry.ssl.truststore.location="$LAB_TRUSTSTORE" \
       --property schema.registry.ssl.truststore.password="$LAB_TRUSTSTORE_PASSWORD" \
       --property schema.registry.ssl.truststore.type=JKS \
       ...
       --producer-property ssl.truststore.location="$LAB_TRUSTSTORE" \
       --producer-property ssl.truststore.password="$LAB_TRUSTSTORE_PASSWORD" \
